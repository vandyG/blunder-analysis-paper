...
\begin{itemize}
    ...
    \item \textbf{Player ratings:} the player's Elo values are recorded as integer fields for White and Black. The reported rating changes are stored separately.
    ...
    \item \textbf{Engine-based evaluations:}\label{sub:engine_eval}
        \begin{itemize}
            \item \texttt{cp\_score}: a centipawn-style score (White-perspective). Mate scores are converted to large numeric constants so the field remains numeric for vectorized pipelines.
...
\begin{itemize}
    ...
    \item Evaluation features: \texttt{cp\_score} and \(\Delta\mathrm{eval}_t\), optionally winsorized to limit the influence of extreme mate constants (See Section~\ref{sub:engine_eval}).
...
\begin{enumerate}
    ...
    \item \textbf{Evaluation score scaling:} The centipawn score \texttt{cp\_score} exhibits extreme values in mate positions (set to large numeric constants such as $\pm10000$). To prevent these outliers from dominating distance-based analyses, we apply a bounded transformation:
    \[
...
Principal Component Analysis (PCA) is a fundamental tool for identifying directions of maximum variance in high-dimensional data and for reducing feature dimensionality while retaining explanatory power. However, standard implementations require the full data matrix $X \in \mathbb{R}^{n \times d}$ to be loaded into memory. For our datasets, the size of $X$ exceeds available memory.
